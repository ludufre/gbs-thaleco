...
const StructParser = {
  ...
  getValue(buff: any[], structItem: { type: "byte" | "string" | "uint16le"; size: number }) {
  ...
  gbsPrompt(
    ...
    .then((password: string) => {
      ...
      const checkStatus = (r: Response) => {
        if (r.status === 429) {
          return r.json().then((ms: number) => {
            const err: any = new Error(
              `Muitas tentativas. Aguarde ${Math.ceil((ms || 0) / 1000)}s antes de tentar novamente.`
            );
...
const rejectRestore = (message: string) => {
...
const doRestore = (file: ArrayBuffer) => {
  ...
  for (let i = 0; i < BACKUP_MAGIC.length; i++) {
    if (fileBuffer[i] !== BACKUP_MAGIC[i]) {
      rejectRestore(
        "Invalid or legacy backup format.\n" +
  ...
  if (formatVersion !== BACKUP_FORMAT_VERSION) {
    rejectRestore(
      "Unsupported backup format version: " + formatVersion + " (expected " + BACKUP_FORMAT_VERSION + ")"
    );
    return;
  }
  const expectedCrc =
  ...
  if (bkMajor !== FW_MAJOR || bkMinor !== FW_MINOR || bkPatch !== FW_PATCH) {
    console.warn(
      `Backup was exported from firmware v${bkMajor}.${bkMinor}.${bkPatch}, ` +
...
const gbsPrompt = (text: string, defaultValue = "", inputType = "text") => {
