const Structs = {
    slots: [
        ...
        { name: "advCombLumaModeNTSC", type: "byte", size: 1 },
        { name: "advCombChromaModeNTSC", type: "byte", size: 1 },
        { name: "advCombChromaTapsNTSC", type: "byte", size: 1 },
        { name: "advCombLumaModePAL", type: "byte", size: 1 },
        { name: "advCombChromaModePAL", type: "byte", size: 1 },
...
const createWebSocket = () => {
    ...
    GBSControl.ws.onmessage = (message) => {
        ...
        if (messageDataAt0 === "$") {
            ...
            const syncStripperChar = message.data[18] || "1"; // Default 1 (ON)
            // Comb Control parameters (positions 19-24)
            const combLumaNChar = message.data[19] || "0"; // NTSC Luma mode
            const combChromaNChar = message.data[20] || "0"; // NTSC Chroma mode
            const combTapsNChar = message.data[21] || "2"; // NTSC Chroma taps
            ...
            const btnSmooth = document.getElementById("gbs-pro-smooth");
            if (btnSmooth) {
...
const updateSlotNames = () => {
    ...
    for (let i = 0; i < GBSControl.maxSlots; i++) {
        ...
        el.setAttribute("gbs-meta", getSlotPresetName(parseInt(structs.slots[i].presetID, 10)));
...
const initVideoFilters = () => {
    ...
    if (lumaDecBtn)
        lumaDecBtn.addEventListener("click", () => updateLumaMode(-1));
    if (lumaIncBtn)
        lumaIncBtn.addEventListener("click", () => updateLumaMode(1));
    // Chroma Mode +/- buttons
    ...
    if (chromaDecBtn)
        chromaDecBtn.addEventListener("click", () => updateChromaMode(-1));
    if (chromaIncBtn)
        chromaIncBtn.addEventListener("click", () => updateChromaMode(1));
    // Chroma Taps +/- buttons
    ...
    if (tapsDecBtn)
        tapsDecBtn.addEventListener("click", () => updateChromaTaps(-1));
    if (tapsIncBtn)
        tapsIncBtn.addEventListener("click", () => updateChromaTaps(1));
};
